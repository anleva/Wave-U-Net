...
from keras.layers import Input, Conv1D, LeakyReLU
...
class Wnet:
    ...
    def __init__(self):
        ...
        self.u_net = [
            {'f': 24*1, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*2, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*3, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*4, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*5, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*6, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*7, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*8, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*9, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*10, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*11, 'p': 2, 'k_down': 15, 'k_up': 5},
            {'f': 24*12, 'p': 2, 'k_down': 15, 'k_up': 5},
        ]
    ...
    def build_models(self, verbose=True):
        ...
        for i in range(len(self.u_net)):
            ...
            x = MaxPooling1D(
                name='U_Down_Decimate_{}'.format(i),
                pool_size=1,
                strides=2,
                padding='same',
            )(x)
        ...
        ########################
        self.wave_net_model = Model(x_mix, x_sources, name='wave_net')
        self.wave_net_model.compile(
            optimizer=Adam(lr=0.0001),
            loss=cropped_mean_squared_error
        )
        if verbose:
            self.wave_net_model.summary()
    ...
    def load_model(self, model_file_name=''):
        ...
        self.wave_net_model = load_model(model_file_path)
    ...
    def load_model_weights(self, weights_file_name='', last=True):
        ...
        try:
            ...
            self.wave_net_model.load_weights(model_path, by_name=True)
    ...
    def save_model(self, save_examples=True):
        ...
        file_name = 'wave_net_model_{}.h5'.format(time_stamp)
        model_path = os.path.join(self.models_path, file_name)
        self.wave_net_model.save(model_path)
    ...
    def save_example_np(self, time_stamp=None, nb_examples=10):
        ...
        y_sources = self.wave_net_model.predict(x_mix)

        # Save
        if time_stamp is None:
            time_stamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        path_anfang = os.path.join(self.examples_path, 'wave_net_model_{}'.format(time_stamp))
    ...
    def train(self, epochs_between_saves=10, number_of_saves=1, batch_size=16):
        ...
        print('Training job for model wave_net.')
            ...
            try:
                with open('settings.json', 'r') as fp:
                    ...
                    if 'learning_rate' in settings.keys():
                        K.set_value(self.wave_net_model.optimizer.lr, settings['learning_rate'])
